...
const fetchBannerDetails = async () => {
  try {
    ...
    if (res.ok) {
      ...
      bannerType.value = data.type || 'none'
      url.value = data.url || ''
      categoryId.value = data.category_id || null
      productId.value = data.product_id || null
      durationType.value = data.duration_type || null
      durationValue.value = data.duration_value || null
      sortOrder.value = data.sort_order || 0
      imagePreviewUrl.value = data.image_url || null
    }
  } catch (error) {
...
const submitForm = async () => {
  ...
  // If edit and no new image, we don't strictly *need* an image file. But form valid checks it.
  // We'll skip image validation if imagePreviewUrl is present (meaning it already has an image)
      ...
      <VCard rounded="lg" elevation="0">
        <VCardItem class="py-4 px-6 border-b">
          ...
          <VCardSubtitle class="mt-1">
            قم بتعديل بيانات هذا البانر وتحديث الصورة إن أردت.
          </VCardSubtitle>
        </VCardItem>

        <VAlert v-if="submitSuccess" type="success" variant="tonal" class="ma-6" closable>
          تم تعديل البانر بنجاح! جاري التوجيه للبانرات...
        </VAlert>
        ...
        <VCardText class="pa-6">
          <VForm ref="formRef" @submit.prevent="submitForm">
            <VRow>
              <VCol cols="12">
                <p class="text-body-1 font-weight-medium mb-3">صورة البانر</p>>
            ...
            <VCardActions class="px-0 pt-6 mt-4 border-t">
              ...
              <VSpacer />
              <VBtn type="submit" variant="elevated" color="primary" rounded="lg" class="px-8" :loading="isLoadingSubmit" :disabled="isLoadingSubmit" prepend-icon="ri-save-3-line">حفظ التعديلات</VBtn>
            </VCardActions>
          </VForm>
        </VCardText>
      </VCard>
